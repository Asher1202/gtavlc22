...
Dialog:DIALOG_FDEPOSIT1(playerid, response, listitem, inputtext[]) 
{
	...
	switch(SelectedItem[playerid]) {
		case 0: {
			...
			format(string2, sizeof(string2), "* %s da gui $%s vao ket an toan cua to chuc %s.", GetName(playerid), FormatNumber(strval(inputtext)), FactionName(PlayerInfo[playerid][pMember]));
			Log("logs/facsafe.log", string2);	
		}
		case 1: {
			...
			format(string2, sizeof(string2), "* %s da gui %s vat lieu vao ket an toan cua to chuc %s.", GetName(playerid), FormatNumber(strval(inputtext)), FactionName(PlayerInfo[playerid][pMember]));
			Log("logs/facsafe.log", string2);	
		}
		case 2: {
...
Dialog:DIALOG_FWITHDRAW1(playerid, response, listitem, inputtext[]) 
{
	...
	switch(SelectedItem[playerid]) {
		case 0: {
			...
			SendClientMessage(playerid, COLOR_MONEY, string);
			Update(playerid, pCashx);
			format(string2, sizeof(string2), "* %s da lay $%s tu ket to chuc %s ! So con lai ben trong la: $%s.", GetName(playerid), FormatNumber(strval(inputtext)), FactionName(PlayerInfo[playerid][pMember]), FormatNumber(SafeInfo[SeifID[playerid]][sMoney]));
			Log("logs/facsafe.log", string2);	
		}
		case 1: {
			...
			SafeInfo[SeifID[playerid]][sMaterials] -= strval(inputtext);
			PlayerInfo[playerid][pMats] += strval(inputtext);
			format(string, sizeof(string), "%s Ban lay tu ket sat to chuc %s vat lieu! So con lai ben trong la %s vat lieu.",GetName(playerid), FormatNumber(strval(inputtext)), FormatNumber(SafeInfo[SeifID[playerid]][sMaterials]));
			SendClientMessage(playerid, COLOR_MONEY, string);
			format(string, sizeof(string), "UPDATE `users` SET `Materials`='%d' WHERE `ID`='%d'", PlayerInfo[playerid][pMats], PlayerInfo[playerid][pSQLID]);
			mysql_query(SQL, string, false);
			//InsertLog(playerid, string, LOG_DEPOSIT);	
			format(string2, sizeof(string2), "* %s da lay %s vat lieu tu ket to chuc %s ! So con lai ben trong la: %s.", GetName(playerid), FormatNumber(strval(inputtext)), FactionName(PlayerInfo[playerid][pMember]), FormatNumber(SafeInfo[SeifID[playerid]][sMaterials]));
			Log("logs/facsafe.log", string2);	
		}
		case 2: {
...
Dialog:DIALOG_TRUNGTHU(playerid, response, listitem ,inputtext[]) {
	...
	switch(listitem) {
		...
		case 1: {
			...
			format(szDialog, sizeof szDialog, "$2.000.000\t150 Banh\n500 Xu\t3000 Banh\nXe Trung Cap Ngau Nhien\t5000 Banh\n");
			strcat(szDialog2, szDialog);
			format(szDialog, sizeof szDialog, "1 Chiec Xe Trung Cap Tuy Chon\t7500 Banh\nBOOMBOX\t1500 Banh\n100 Banh Hamburger\t50 Banh\n");
			strcat(szDialog2, szDialog);
			format(szDialog, sizeof szDialog, "50 XU\t300 Banh\n10.000 Vat Lieu\t1000 Banh\nThe Doi Ten\t200 Banh\nHop Qua Trung Thu\t500 Banh\n");
			strcat(szDialog2, szDialog);
			format(szDialog, sizeof szDialog, "Hop Qua Epic Car\t5500 Banh\nHop Qua Legend Car\t11500 Banh\nHop Qua Random\t500 Banh\nMau Vip Xe\t200 Banh\n");
			strcat(szDialog2, szDialog);
			format(szDialog, sizeof szDialog, "Sung 200 Vien Dan\t500 Banh\n1 Chiec Xe Cao Cap Ngau Nhien\t12000 Banh\nMot Chiec Xe Cao Cap Tuy Chon\t19999 Banh");
			strcat(szDialog2, szDialog);
...
Dialog:DIALOG_TRUNGTHU2(playerid, response, listitem, inputtext[]) {
	...
	switch (listitem) {
		case 0: {
			if(PlayerInfo[playerid][pBanhTrungThu] < 150) return Dialog_Show(playerid, 0, DIALOG_STYLE_MSGBOX, "Tho Lam Banh", "Ban khong du banh", "Dong y", "Cancel");
		...
		case 2: {
			...
			format(string ,sizeof string, "%s Da Nhan Xe %s Trung Cap Tu Tho Lam Banh (Ngau Nhien)",GetName(playerid),aVehicleNames[D_Vehicles[id][dModel]-400]);
			SendAdminMessage(COLOR_YELLOW, string, 1);
			PlayerInfo[playerid][pBanhTrungThu] -= 5000;
		}
		case 3: {
			if(PlayerInfo[playerid][pBanhTrungThu] < 7500) return Dialog_Show(playerid, 0, DIALOG_STYLE_MSGBOX, "Tho Lam Banh", "Ban khong du banh", "Dong y", "Cancel");
			if(GetSlots(playerid) <= GetVehicles(playerid)) return SendClientMessage(playerid, -1, "Ban da het slot xe! Hay mua mot slot tai /shop.");
		...
		case 10: {
			if(PlayerInfo[playerid][pBanhTrungThu] < 5500) return Dialog_Show(playerid, 0, DIALOG_STYLE_MSGBOX, "Tho Lam Banh", "Ban khong du banh", "Dong y", "Cancel");
			PlayerInfo[playerid][pBanhTrungThu] -= 5500;
		...
		case 11: {
			if(PlayerInfo[playerid][pBanhTrungThu] < 11500) return Dialog_Show(playerid, 0, DIALOG_STYLE_MSGBOX, "Tho Lam Banh", "Ban khong du banh", "Dong y", "Cancel");
			PlayerInfo[playerid][pBanhTrungThu] -= 11500;
		...
		case 13: {
			...
			SetPVarInt(playerid, "tholambanh", 1);
		...
		case 14: {
			if(PlayerInfo[playerid][pBanhTrungThu] < 500) return Dialog_Show(playerid, 0, DIALOG_STYLE_MSGBOX, "Tho Lam Banh", "Ban khong du banh", "Dong y", "Cancel");
			Dialog_Show(playerid,DIALOG_ChonSung,DIALOG_STYLE_TABLIST_HEADERS,"Nhan Sung Tu Tho Lam Banh", "Sung\tDan\nM4\t200 Vien\nAK47\t200 Vien","OK","Cancel");
		}
		case 15: {
			if(PlayerInfo[playerid][pBanhTrungThu] < 12000) return Dialog_Show(playerid, 0, DIALOG_STYLE_MSGBOX, "Tho Lam Banh", "Ban khong du banh", "Dong y", "Cancel");
			if(GetSlots(playerid) <= GetVehicles(playerid)) return SendClientMessage(playerid, -1, "Ban da het slot xe! Hay mua mot slot tai /shop.");
			new x = 0;
			foreach(new id: Cars<2>) x++;
			new rand = random(x);
			new id = Iter_Index(Cars<2>, rand);
			GiveVehicle(playerid, D_Vehicles[id][dModel], true);
			format(string ,sizeof string, "%s Da Nhan Xe %s Cao Cap Tu Tho Lam Banh (Ngau Nhien)",GetName(playerid),aVehicleNames[D_Vehicles[id][dModel]-400]);
			SendAdminMessage(COLOR_YELLOW, string, 1);
			PlayerInfo[playerid][pBanhTrungThu] -= 12000;
		}
		case 16: {
			if(PlayerInfo[playerid][pBanhTrungThu] < 19999) return Dialog_Show(playerid, 0, DIALOG_STYLE_MSGBOX, "Tho Lam Banh", "Ban khong du banh", "Dong y", "Cancel");
			if(GetSlots(playerid) <= GetVehicles(playerid)) return SendClientMessage(playerid, -1, "Ban da het slot xe! Hay mua mot slot tai /shop.");
			new temp[100], strings[4000], price[50], vehname[50];
			strcat(strings, "Model\tPrice\n");
...
Dialog:DIALOG_ChonSung(playerid,response,listitem,inputtext[]) {
	if(!response) return true;
	new string[120];
	switch(listitem) {
		case 0: {
			if(Inventory_GetFreeID(playerid, 1) == -1) Inventory_Add(playerid,"M4A1", 356, 200, 2);
		...
		case 1: {
			if(Inventory_GetFreeID(playerid, 1) == -1) Inventory_Add(playerid,"AK47", 355, 200, 2);
...
Dialog:DIALOG_TRUNGTHUNHANXE2(playerid,response,listitem,inputtext[]) {
	if(!response) return true;
	...
	switch(listitem) {
		...
		case 2: {
			if(PlayerInfo[playerid][pShowCeas] == 0) {
				...
				TextDrawHideForPlayer(playerid, TimeTD[0]);
				TextDrawHideForPlayer(playerid, TimeTD[1]);
				TextDrawHideForPlayer(playerid, TimeTD[2]);
...
Dialog:DIALOG_TRUCKER(playerid, response, listitem, inputtext[]) 
{
	...
	do {
		...
		switch(listitem) {
			...
			case 6: {
				// Stolen cars
				truckX = TruckTrungThuPos[rand][0];
				truckY = TruckTrungThuPos[rand][1];
				truckZ = TruckTrungThuPos[rand][2];
			}					
		}
	}
...
Dialog:DIALOG_REGISTER6(playerid, response, listitem, inputtext[]) 
{
	...
	PlayerInfo[playerid][pQuaTanThu] = 0;	
	format(string, sizeof(string), "UPDATE users SET `Tutorial`='1',`Bank`='20000',`Money`='15000', `QuaTanThu` = '0' WHERE `name`='%s'",GetName(playerid));
	mysql_query(SQL, string);
	format(string, sizeof(string), "%s(%d) da dang ky vao may chu. [#%d]", GetName(playerid), playerid, PlayerInfo[playerid][pSQLID]);
	SendAdminMessage(COLOR_LIGHTRED, string, 1);				
...
Dialog:DIALOG_INVENTORY(playerid, response, listitem, inputtext[]) {
	...
	switch(listitem) {
		...
		case 0: {
			...
			else if(strmatch("Banh Trung Thu", invData[playerid][id][invItem])) {
				if(gettime() < GetPVarInt(playerid, "ChoDoi")) return SCMf(playerid, COLOR_LGREEN, ">> Vui long doi %d giay!", GetPVarInt(playerid, "ChoDoi") - gettime());
				new Float:armour;
				GetPlayerHealthEx(playerid, health);
				GetPlayerArmourEx(playerid, armour);
				if(PlayerInfo[playerid][pHunger] < 100) Stamina_SetValue(playerid, 100);
				Inventory_Remove(playerid, "Banh Trung Thu", GetPVarInt(playerid, "UseNum"));
				NearMessage2(playerid, 25.0,COLOR_PURPLE, "** %s da lay %d cai Banh Trung Thu tu tui ni-lon ra va su dung.", GetName(playerid), GetPVarInt(playerid, "UseNum"));
				ApplyAnimation(playerid,"FOOD","EAT_Burger", 3.0, 0, 0, 0, 0, 0);
				if(health < 90 && InWar[PlayerInfo[playerid][pMember]] == 1) {
			...
			// else if(strmatch("Sailor Moon", invData[playerid][id][invItem])) {
			// 	if(gettime() < GetPVarInt(playerid, "Ban2Ca")) NightBot(playerid, "Ban 2 ca");
			// 	if(TimpMateriale[playerid] == 1) return Dialog_Show(playerid, 0, DIALOG_STYLE_MSGBOX, "Ban ca", "ERROR: Ban khong the ban mot con ca qua nhanh, hay tro lai sau!", "Ok", "Cancel");
			// 	if(BizzInfo[InBussines[playerid]][bType] == 6)
			// 	{
			...
			// 		pFishing[playerid] = 0;
			// 		x = 2022;
			// 		format(strings, sizeof(strings), "{FF0000}>>{FFFFFF} %s (%d) da ban mot con {FFF400}Sailor Moon{FFFFFF} voi gia ({15FF00}$%s{FFFFFF}).", GetName(playerid), playerid, FormatNumber(x));
			// 		NearMessage(playerid, COLOR_YELLOW, strings);	
			// 		GuiDenAdminVaHelper(COLOR_YELLOW, strings, 1); 
			// 		FishPrice[playerid] = x;
			// 		SetPVarInt(playerid, "Ban2Ca", gettime() + 25);
